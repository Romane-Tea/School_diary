...
function access_ok() {
  ...
  // 一度にデータを取得
  let range = sheet.getRange(3, 3, last_row - 2, 1);
...
function GET_SHEET_NAME(sheet_no) {
  try{
    const sheet_name = SpreadsheetApp.getActive().getSheets()[sheet_no - 1].getSheetName();
  }catch(e){}
  return sheet_name
}
...
function onEdit(e) {
  ...
  if (value === "") { // H列から値が削除された場合、I列から時刻を削除
    ...
    let time = Utilities.formatDate(new Date(), "JST", "H:mm");
...
function GetSpreadsheet(sheet_name) {
  ...
  let last_row = sheet.getLastRow();     // 最終行取得
  let data = sheet.getRange(1, 1, last_row, last_col).getDisplayValues();
